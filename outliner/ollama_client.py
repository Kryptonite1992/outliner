...
class Outliner:
    ...
    def chat(self, messages, temperature=0.1, top_p=0.9, frequency_penalty=0.0, presence_penalty=0.0):
        """
        向 Ollama 0.6b 发送对话消息。
        messages: List[dict]，如 [{"role": "user", "content": "..."}]
        temperature: 控制随机性，0-2之间
        top_p: 核采样参数，0-1之间
        frequency_penalty: 频率惩罚，-2.0到2.0之间，减少重复内容
        presence_penalty: 存在惩罚，-2.0到2.0之间，鼓励新话题
        返回字符串回复。
        """
    ...
    def extract_paragraph_title(self, paragraph: str) -> str:
        ...
        使用较高的frequency_penalty来避免重复词汇，提高标题质量。
        """
        prompt = (
        ...
        # 使用较高的frequency_penalty来减少重复，提高标题的准确性和简洁性
        reply = self.chat(messages, temperature=0.2, frequency_penalty=0.8, presence_penalty=0.3)
        # 可根据需要做进一步清洗
        return reply.strip()
